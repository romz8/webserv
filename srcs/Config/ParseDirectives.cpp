...
/*   Updated: 2024/05/16 20:50:27 by rjobert          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ParseDirectives::parseServerConfig( Directives *d, StrVector & content, int n_line ) {
	int last = check_brackets(content, n_line);
	StrVector line;
	string content_parsed;
	ServerConfig *serv = new ServerConfig; //change to ServerConfig

	try {
		serv->parse_ServerConfig_line(content[n_line]);
		...
		d->addServerConfig(serv);
		d->dirSet["ServerConfig"] = true;
	}
	catch (exception &e)
	{
		delete serv;
...
void	ParseDirectives::save_ServerConfig_name( Directives *d, StrVector & line ) {
	if (line.size() < 2)
		throw logic_error("Unexpected amount of arguments");
	...
	for (size_t i = 1; i < line.size(); i++)
		d->ServerConfig_names.push_back(line[i]);
}
...
int	ParseDirectives::parseServerConfigDirectives( Directives *d, StrVector & line, int type, StrVector &content, int n_line ) {
	...
	switch (type)
	{
		...
		case 2:
			...
			break ;
		case 3:
			ParseDirectives::save_ServerConfig_name(d, line);
			break ;
		case 4:
		...
		case 7:
			...
			break ;
		default:
			throw logic_error("\"" + line[0] + "\" not a valid directive for ServerConfig");
	}
	return (n_line);
}
